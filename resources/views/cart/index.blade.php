<x-tailwind></x-tailwind>
<div class="overflow-x-auto w-full">
    <table class="table w-full h-3/4">
        <!-- head -->
        <thead>
            <tr class="">

                <th>Name</th>
                <th>Price</th>
                <th>Quantity</th>
                <th>Action</th>
            </tr>
        </thead>
        <tbody>
            <!-- row 1 -->
            @if (session()->has('success'))
                <div class="alert alert-success shadow-lg w-1/4 overflow-hidden">
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6" fill="none"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span>{{ session()->get('success') }}</span>
                    </div>
                </div>
            @endif
            @php
                $total = 0;
            @endphp

            @foreach ($carts as $cart)
                <tr class="">
                    @php
                        $total += $cart->product->price * $cart->quantity;
                    @endphp
                    <td>
                        <div class="flex items-center space-x-3">
                            <div class="avatar">
                                <div class="mask mask-squircle w-12 h-12">
                                    <img src="{{ asset('assets/Logo.png') }}" />
                                </div>
                            </div>
                            <div>
                                <div class="font-bold">{{ $cart->product->name }}</div>

                                <div class="text-sm opacity-50"></div>

                            </div>
                        </div>
                    </td>
                    <td>
                        Rp. {{ number_format($cart->product->price) }}
                        <br />
                        @foreach ($cart->product->categories as $category)
                            <span class="badge badge-ghost badge-sm">{{ $category->name }}</span>
                        @endforeach
                    </td>
                    <td>{{ $cart->quantity }}</td>
                    <th>
                        <form action="{{ route('cart.destroy', $cart->product_id) }}" method="post">
                            @csrf
                            @method('delete')
                            <button type="submit">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                </svg>
                            </button>
                        </form>
                    </th>
                </tr>
            @endforeach
        </tbody>
        <!-- foot -->
        <tfoot>
            <tr>
                <th class="text-lg">Total</th>
                <th class="text-lg font-medium">Rp.{{ number_format($total) }}</th>
                <th></th>
                <th>
                    <form action="{{ route('invoice.store') }}" method="post">
                        @csrf
                        <input type="hidden" name="total" value="{{ $total }}">
                        <x-button styles="bg-slate-800 text-white">Checkout</x-button>
                    </form>
                </th>
            </tr>
        </tfoot>

    </table>

</div>
